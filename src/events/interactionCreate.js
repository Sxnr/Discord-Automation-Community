const { Events, ChannelType, PermissionsBitField, ActionRowBuilder, ButtonBuilder, ButtonStyle, EmbedBuilder, MessageFlags, StringSelectMenuBuilder } = require('discord.js');
const discordTranscripts = require('discord-html-transcripts');
const db = require('../database/db');

// ➕ HELPER: Crea el canal del ticket con numeración y categoría
...
module.exports = {
    ...
    async execute(interaction) {
        ...
        // 2. LÓGICA DE MENÚS DESPLEGABLES (HELP + TIPOS DE TICKET)
        ...
        if (interaction.isButton()) {
            try {
                    ...
                    if (settings?.ticket_types) {
                        ...
                        if (types.length > 0) {
                            ...
                            return await interaction.editReply({ 
                                content: '**¿Cuál es el motivo de tu ticket?**',
                                components: [row]
                            });
                        }
                    }
                ...
                if (interaction.customId === 'close_only') {
                    await interaction.deferUpdate();
                    await interaction.followUp({ content: '🔒 Cerrando ticket sin transcript. El canal se eliminará en 5 segundos.' });
                ...
                if (interaction.customId === 'join_giveaway') {
                    // ➕ FIX: Incluir required_role en la consulta
                    ...
                    if (giveaway.required_role) {
                        const hasRole = interaction.member.roles.cache.has(giveaway.required_role);
                        if (!hasRole) {
                            return interaction.reply({
                                content: `❌ Necesitas el rol <@&${giveaway.required_role}> para participar en este sorteo.`,
                                flags: [MessageFlags.Ephemeral]
                            });
                        }
                    }
